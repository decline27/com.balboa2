            ...
            res.on('end', () => {
                try {
                    ...
                    if (data.includes('<error>')) {
                        console.error(`[BWA Cloud] API Error Response: ${data}`);
                    }
                    resolve(data);
                }
            });
...
function extractDataFromXML(xmlString) {
    ...
    if (xmlString.includes('<error>')) {
        ...
        const errorMsg = errorMatch ? errorMatch[1] : 'Unknown Cloud Error';
        console.error(`[BWA Cloud] XML Error found: ${errorMsg}`);
